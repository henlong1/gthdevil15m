...
// ==========================================================
let gattahuCurrentLevel = 15;
let gattahuCurrentStage = 1;
let gattahuCurrentBetUnit = 0;
let gattahuTotalUnits = 15; // Starting units set to 15 from level 15
...
const gattahuLevelMap = {
    ...
    27: {
        ...
        4: { bet: 9, win: { type: 'goto', level: 30 }, lose: { type: 'gotoLevel', level: 12 } } // Single bet
    },
    28: {
        ...
        4: { bet: 8, win: { type: 'goto', level: 30 }, lose: { type: 'gotoLevel', level: 14 } } // Single bet
    },
    29: {
        1: { bet: 1, win: { type: 'goto', level: 30 }, lose: { type: 'goto', stage: 2 } }, // Single bet
        2: { bet: 2, win: { type: 'goto', level: 30 }, lose: { type: 'goto', stage: 3 } }, // Single bet
        3: { bet1: 3, bet2: 1, win: { type: 'calcLevelSumCurrentLevelSubtract', units: [3, 1], subtract: 3 }, lose: { type: 'goto', stage: 4 } },
        4: { bet: 7, win: { type: 'goto', level: 30 }, lose: { type: 'gotoLevel', level: 16 } } // Single bet
    },
    30: {
        // Level 30 is the win target level, may not have special betting rules.
        // Or you can define betting rules for the final stage.
        1: { bet: 10, win: { type: 'gameWin' }, lose: { type: 'gameOver' } } // Example: final bet for Level 30
    }
};
...
function gattahuInitializeGame() {
    gattahuCurrentLevel = 15; // Starting level changed to 15
    gattahuCurrentStage = 1;
    gattahuTotalUnits = 15; // Starting units changed to 15
...
function gattahuUpdateDisplay() {
    ...
    if (gattahuTotalUnits >= 30) {
        gattahuGameWin("축하합니다! 총 유닛이 30에 도달하여 게임에 승리했습니다!"); // "Congratulations! Total units reached 30, you won the game!"
    ...
    if (gattahuTotalUnits <= 0 && !(gattahuCurrentLevel === 15 && gattahuCurrentStage === 1 && gattahuTotalUnits === 15)) {
...
function handleGattahuLose() {
    ...
    // Special loss logic for Level 22-26, Stage 4
        ...
        if (unitsLost === stageData.bet1) {
            nextLevel = gattahuCurrentLevel - 15;
            ...
            if (stageData.lose.type === 'specialLoseLevel25_4') {
                nextLevel = gattahuCurrentLevel + 1;
                ...
                nextLevel = gattahuCurrentLevel + 2;
                gattahuMessageEl.textContent = `패배! (2번째 베팅 실패) 레벨 ${nextLevel}로 이동합니다.`; // "Loss! (2nd bet failed) Moving to Level {level}."
